...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=org.connectbot "

INSTRUMENTATION_OPTS=" \
--skip-class=org.connectbot.R \
\
\
"
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.connectbot.debug$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.connectbot.debug.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall org.connectbot.debug > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall org.connectbot.debug.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" -eq "0" ]; then \
	...
	$(ADB) shell pm clear org.connectbot.debug > /dev/null; \
	fi)
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assembleOssDebug </dev/null && \
	mv app/build/outputs/apk/oss/debug/app-oss-debug.apk app-debug.apk && \
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) org.connectbot.tests/org.connectbot.ConnectbotJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces org.connectbot.debug ./traces/$(TEST_NAME) force-clean
...
# Ensure the right apk are installed and optionally start the emulator. Clean up the data if necessary.
	...
	$(GW) --info -PjacocoEnabled=true -PcarvedTests=false clean jacocoGUITestCoverage 2>&1 </dev/null | tee espresso-tests-coverage.log
...
carved-tests.log : .carved-all
	...
	$(GW) -i -PjacocoEnabled=true -PcarvedTests=true clean jacocoUnitTestCoverage 2>&1 </dev/null | tee carved-tests.log
